...
namespace g3
{
    ...
    public partial class DMesh3 : IDeformableMesh
    {
        ...
        public IEnumerable<Vector3d> Normals()
        {
            ...
            {
                int i = 3 * vid;
                yield return new Vector3d(normals[i], normals[i + 1], normals[i + 2]);
            }
        }
        ...
        /// <summary>
        /// Get all triangles. Usually needed for mesh construction in Unity
        /// </summary>
        public int[] GetAllTriangles()
        {
            var trianglesIndexes = new int[TriangleCount * 3];
            for (var i = 0; i < trianglesIndexes.Length; i += 3)
            {
                trianglesIndexes[i] = triangles[i];
                trianglesIndexes[i + 1] = triangles[i + 1];
                trianglesIndexes[i + 2] = triangles[i + 2];
            }
